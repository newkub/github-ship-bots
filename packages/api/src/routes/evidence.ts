...
const evidenceSchema = z.object({
  ...
  data: z.string().min(1).max(MAX_EVIDENCE_SIZE),
...
async function verifyCardExists(db: D1Database, cardId: string | undefined) {
  if (!cardId) return true;
  const row = await db.prepare("SELECT id FROM cards WHERE id = ?").bind(cardId).first<{ id: string }>();
  return !!row;
}
function tryDecodeData(data: string): Uint8Array | Error {
  try {
    return base64ToBytes(data);
  } catch (error) {
    return error instanceof Error ? error : new Error("Invalid evidence data");
  }
}
async function safeStoreEvidence(
  env: { EVIDENCE_BUCKET: R2Bucket; DB: D1Database },
  body: { cardId?: string; kind: string; data: string; ciRunUrl?: string }
): Promise<{ id: string; key: string; hash: string } | { error: string }> {
  const cardExists = await verifyCardExists(env.DB, body.cardId);
  if (!cardExists) return { error: "card not found" };
  const decoded = tryDecodeData(body.data);
  if (decoded instanceof Error) return { error: `invalid data: ${decoded.message}` };
  const bytes = decoded;
...
evidence.post("/", async ({ request, set, env, body }) => {
  ...
  const result = await safeStoreEvidence(env, body);
  if ("error" in result) {
    set.status = result.error === "card not found" ? 404 : 400;
