...
- packages/bot         GitHub App worker (Probot/Octokit) on Cloudflare Workers
...
| 3 | GitHub App events configured + old worker deleted | pending (needs GitHub App UI + Cloudflare dashboard) |
...
|-----|------------|-------|--------|
| 1 | `STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET` | `packages/api` | pending |
| 2 | GitHub OAuth App client ID/secret for WorkOS social login | `packages/api` `/auth/callback` | pending |
| 3 | Cloudflare D1/R2/KV bindings for `packages/api` | `packages/api` `wrangler.toml` | pending |
| 4 | GitHub App `wrikka-ship-bot` events: Issues, Issue comment, Pull request | `packages/bot` | pending |
